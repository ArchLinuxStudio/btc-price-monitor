...
- **动态自选目录**：默认保留且不可删除 BTC / ETH，可搜索并添加任意数量的 Coinbase 在线 `*-USD` 现货，或 Bybit / Gate 官方目录中的股票相关 USDT 永续；应用不设置自选数量上限。主行情中的每一行都可拖动排序，也可按 `Alt + ↑/↓` 移动，顺序与精确交易所代码保存在本机。
...
- **交互式 K 线查看**：单击任一行情行或按 `Enter` / `Space` 会打开独立的工作区最大化蜡烛图，主监视器继续显示；可切换 `1m`、`5m`、`15m`、`1h`、`1d`。默认显示最近 120 根（不足时显示已有数据）；滚轮或 `＋/−` 可放大细节或缩小间距查看更多历史，按住图表左右拖动可平移。十字线随指针显示价格和对应 K 线的 UTC 时间标记。图表会提前缓冲当前视图左侧约 480 根历史，通常打开后缓存约 720 根；预加载逐页间隔至少一秒，遇到空页、错误或限流会暂停。缩小或向过去平移超出已加载范围时，会按需加载更早数据，当前图表最多缓存 4,800 根。图表严格读取点击时正在显示的交易所，仅支持 Coinbase / Bybit / Gate 历史数据；若当前热备来自 Kraken / Bitstamp / Bitfinex，会明确提示暂不支持，不会静默换源。
- **图表边界浏览**：可向两端拖出留白，直到最新一根移到左侧或最旧一根移到右侧；始终保留一根完整 K 线，水平间距不变。只有确认到达该数据源最早可查询的 K 线时，日期轴才显示“历史起点”旗标并停止继续加载；普通分页边界和缓存上限不会被标为起点。日期、价格两轴采用更醒目的半粗体数值，空白处不显示虚构日期。
- **当前价格标线**：同源实时报价同步更新当前 K 线及右侧价格标签，水平虚线始终对齐最新 K 线的收盘价，按该根 K 线涨跌呈现红绿颜色。跨周期时从交易所补入实际新 K 线；数据尚未返回则显示“同步 K 线…”，报价滞后时改为灰色并明确提示。查看旧历史时保留历史价格比例，最新价格超出可视范围则在轴边显示方向箭头。
- **超紧凑界面**：宽度始终为 `208px`，默认两行时仅 `208 × 92`；四行以上默认在内部滚动，也可拖动底部继续增高，最多延伸到当前屏幕工作区底部或完整容纳全部自选（取较小值）。超过当前屏幕可见高度的自选继续滚动，不受 8 行限制。添加/删除面板只在点击 `+` 时临时替换行情区，并继续封顶 `170px`。
...
窗口标题栏的 `+` 用于搜索、添加或删除自选品种；例如搜索 `MU` 可在交易所当前上架时添加 `MU.P`。在行情区单击一行（键盘可按 `Enter` / `Space`）即可查看 K 线。图表上可滚轮缩放、按住拖动平移、双击或点击“重置”回到最近 120 根；聚焦图表后也可用 `+`/`-` 缩放、`←`/`→` 平移、`Home` 跳到已加载历史开头、`End` 回到最新位置、`0` 重置。需要继续查询更早历史或加载失败时，可点击“继续加载”；已有图表会保留。图表窗口关闭、按 `Esc` 或 Alt+F4 时只会隐藏，主监视器不会随图表打开或关闭而消失。右侧关闭按钮只把监视器隐藏到系统托盘，程序仍保持运行，恢复窗口后会自动继续刷新行情。右键单击托盘图标并选择“关于 Crypto Top”可查看当前版本、应用图标和简要 GPL 许可信息；点击 GitHub 图标会使用系统默认浏览器打开项目仓库。选择“退出 Crypto Top”才会完全退出。Windows 可能会根据系统设置把新托盘图标放进 `^` 隐藏图标区域。

## macOS / Linux

源代码完全共用，但安装包必须在目标系统上构建。macOS 安装包最低要求 macOS 12.0。仓库内的 `.github/workflows/build-desktop.yml` 可以生成：
