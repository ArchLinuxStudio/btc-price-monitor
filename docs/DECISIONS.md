...
**Implications:** Internal scrolling remains whenever content actually overflows. Layout changes require a real rendered image when presenting a prototype. Native layout acceptance includes drag growth/shrink, fixed width and the management cap; effective 2026-09-10, provide a local installer for the user to verify those interactions, following the non-disruptive validation decision below. Automated checks must not take over the user's desktop, and browser evidence alone must not be labeled native acceptance.
...
**Status:** Accepted; explicit user request on 2026-09-08, with history pacing revised on 2026-09-10
...
After installing the current navigation instance, target two viewport widths of candles before the visible left edge (`ceil(viewport.count × 2)`, clamped to 480–1,920). A normal first page of 240 candles with the latest 120 visible therefore still warms to 720 cached candles. Fetch each speculative page after a 250ms delay, with no overlapping history requests. Explicit older-range demand reuses the same flight and has priority; each received page renders progressively and shifts candle indices plus any drag origin together without changing their anchors. Preserve the four-page demand batch and 4,800-candle cache cap. If all four pages each add 240 real candles and advance the cursor, automatically continue remaining demand after 500ms and retain loading status while queued. Stop speculative scanning after an empty/nonadvancing page, an error, or four insufficient sparse pages; full productive pages do not consume that sparse budget, and a successful explicit demand page may resume prefetch. Sparse/empty/failed demand batches retain explicit continuation instead of automatic retry. HTTP 429/403 pause older requests in the current navigation instance for 60 seconds/10 minutes, respectively, without automatic retries. Reset, reverse navigation, End and cancellation revoke obsolete queued demand; product/interval reload and hiding cancel timers and requests.

**Reason:** Inspection needs precise time/price guides, and an advance buffer removes network waits from ordinary zoom/pan. On 2026-09-10 the user reported slow batches during zoom-out and requested smoother history browsing. The original fixed 480-candle buffer covered progressively less of a wide viewport, the one-second speculative delay slowed replenishment, and unconditional pauses after four successful pages interrupted valid large navigation requests. The revised buffer and paced productive continuation address those causes without changing provider page sizes, concurrency or the real-candle cache limit. Sparse scans and errors still require bounded recovery; prefetch still stops at the selected view's buffer rather than fetching all exchange history.
...
**Implications:** Preserve bounded same-source pagination and prefetch. After a failed/nonadvancing demand page or an unfinished batch containing sparse/empty pages, continuing pointer movements must not restart requests automatically. The 2026-09-10 pacing revision allows only fully productive four-page batches to continue automatically. Explicit “继续加载” can retry paused demand subject to the existing provider cooldown; returning to a loaded range or resetting permits a later new demand. Tests cover both one-candle edges, sparse/no-more-history/cache-limit states, pending zoom and prepend anchors, held-pointer retry suppression, and time-axis/crosshair behavior in blank space. This supersedes the earlier pan constraint that the full viewport had to remain within loaded candles; it does not change native permissions or market semantics.
...
**Constraints:** Origin metadata is requested serially only on the first empty Coinbase/Gate page, cached within the frozen selection/interval, and failed probes remain unknown without automatic retries. The 2026-09-10 smooth-history request supersedes probing on a nonempty short page: usable candles are delivered immediately, and the later empty scan can still establish the same conservative origin. Metadata 429/403 preserve existing candles and impose the existing 60-second/10-minute cooldown once. The actual chart loader rejects partially malformed candle pages so discarded rows cannot later masquerade as the first candle; the older array-returning helper retains its compatibility behavior. No provider, symbol guess, key, CSP origin, or native permission is added.
...
**Presentation:** Date and price axes retain readable 12px semibold values, matching measured crosshair fonts and expanded margins. The subsequent TradingView-style decision updates the palette and grid tick spacing. Preserve compact-window label bounds and real UTC dates.
...
## Decision: Match TradingView's dark chart layout within the local feature set

**Status:** Accepted; explicit user restyle request, implemented 2026-09-11

**Decision:** Use a flat dark chart workspace with a 46px instrument/interval header, 44px left tool rail, in-pane OHLC legend and floating bottom zoom/reset controls. Retain all five supported intervals and the 248px header clearance at widths of at least 960px for the default top-right monitor. The rail exposes working crosshair/grid toggles and earliest-loaded/latest navigation. Follow the layout hierarchy in TradingView's official [UI elements](https://www.tradingview.com/charting-library-docs/latest/ui_elements/) and [Legend](https://www.tradingview.com/charting-library-docs/latest/ui_elements/Legend/) references using the existing local HTML/CSS/canvas implementation.

**Data presentation:** The legend shows the actual latest retained candle when idle, the real candle under the enabled crosshair while inspecting, and dashes in blank margins. It identifies the exact source, instrument and interval; the axis retains the correct USD/USDT unit. Pointer inspection updates the legend and overlay without repainting the candle canvas. The `#131722` pane, subdued gray axes/grid and blue active controls frame matching green `#26a69a` / red `#ef5350` candles, OHLC values and current-price markers. Retain measured 12px semibold axis/crosshair fonts and legible margins. Use bounded decimal price ticks with 1/2/2.5/5/10 steps based on plot height, without altering the real OHLC scale or candle geometry.

**Reason:** The user wants a close visual resemblance to TradingView. Its compact toolbar, side rail and in-chart data hierarchy suit this chart while preserving the monitor's small static application and the existing smooth-history behavior.

**Rejected alternatives:** Decorative drawing/indicator/trading buttons without implemented behavior; embedding TradingView branding or a remote chart; adding a chart library, CDN, framework or bundler; changing the exact-source contract; widening CSP/native permissions; or fabricating candles to fill the chart.

**Implications:** Keep `chart.html`, `chart.css`, `chart.ts` and `chart-price-axis.ts` synchronized with the OHLC, controls, label-bounds and numeric regressions. Preserve bounded history, pending zoom anchors, current-price synchronization and native window behavior. Validate through non-disruptive automated checks and provide a local Windows installer for user-performed visual acceptance; do not drive desktop windows or launch the installer. This local design task does not authorize a commit, push or public Release.
...
**Implications:** The shared application, test, and build configs inherit `ES2025`; a static regression locks it together with the macOS 12.0 floor. TypeScript target/lib settings do not polyfill APIs or guarantee that the system WKWebView shipped with every macOS 12 point release implements all ES2025-era features. Review new syntax and runtime APIs individually and retain fallbacks where needed. Keep automated deployment-floor checks; arrange minimum-version runtime acceptance through user testing without taking over their desktop, and record unverified runtime coverage honestly. A future compiler upgrade must not silently move this fixed target.

## Decision: Validate without interrupting the user's desktop

**Status:** Accepted; explicit standing user preference on 2026-09-10

**Decision:** Do not perform UI tests that control desktop windows, focus, mouse or keyboard, or otherwise interfere with the user's normal computer use. Continue appropriate unit tests, type checks, builds and headless checks that do not disturb the desktop. For interactive/native acceptance, provide a local installer and let the user install and verify it themselves. Do not launch that installer or drive the installed app for testing unless the user later explicitly requests those interactions.

**Reason:** The user considers disruptive computer-control tests unnecessary and prefers to validate a local package personally.

**Implications:** This supersedes older mandatory agent-driven native smoke tests. A local build is not a public release and does not authorize installation. Report package/build success separately from user acceptance, which stays pending until feedback; do not block delivery on taking over the desktop. Historical native smoke evidence remains historical. Follow the independent standing preference against downloading/verifying GitHub-built attachments.
...
**Decision:** A source push is not a release. Formal releases use a new SemVer tag, four build runners, five desktop assets and correct UTF-8 text. Confirm successful CI and Release publication. Effective 2026-09-10, the user's standing preference supersedes independent post-upload attachment verification: do not download GitHub-built attachments or separately check their downloads, sizes, digests, file signatures or package metadata unless explicitly requested later. Existing automated workflow checks remain in place.
...
**Implications:** Follow [`RELEASE.md`](RELEASE.md). Never overwrite or move a published tag or Release. Each subsequent formal release requires a new version and tag; the current release/source relationship belongs in `CURRENT_STATE.md`. Historical attachment verification does not authorize repeating it in future releases.
