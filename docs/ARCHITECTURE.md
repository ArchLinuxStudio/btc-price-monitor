...
Selected-source WebSocket / REST fallback ── ChartCurrentPrice ── chart price overlay
...
| `src/chart-viewport.ts` | Pure floating candle viewport math: newest-120 reset, minimum-12 zoom, proportional geometry, two-sided blank pan with one complete edge slot retained, and safe real-data bounds |
