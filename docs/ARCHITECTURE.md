...
| `src/chart.html` / `src/chart.css` / `src/chart.ts` | Local full-work-area candlestick view with flat dark toolbar/tool rail, real OHLC legend, interval/viewport controls, DPI-aware canvas renderer, pointer/wheel/keyboard interaction, status/retry presentation, and chart-window lifecycle |
...
| `src/chart-price-axis.ts` | Bounded readable price ticks using decimal steps and available plot height, without changing the actual visible price scale |
...
- The chart uses a TradingView-style flat dark workspace: a 46px header with instrument, market class and five interval buttons; a 44px left rail; a `#131722` pane; and a compact footer. Rail buttons toggle crosshair/grid and invoke earliest-loaded/latest navigation. The floating bottom controls retain anchored zoom and newest-120 reset. These are implemented local controls; there are no placeholder drawing, indicator or trading tools, remote chart library or TradingView branding.
- The top 76px of the canvas layout is reserved for the in-pane instrument/interval/source and real OHLC legend. The legend normally shows the latest retained candle, uses the actual candle under the enabled crosshair while inspecting, and shows dashes in blank margins. Leaving inspection restores the latest values; pointer motion updates only the crosshair overlay and legend. The axis currency distinguishes USD from the approved USDT perpetual category.
...
- `ChartNavigation` separates the requested range from the currently renderable loaded range. Zoom-out or pan beyond loaded history requests older pages from the same frozen product/source/interval, at most four serial pages per batch and 4,800 cached candles per chart selection/interval. Each page returns at most 240 candles: Coinbase/Gate scan 240-bucket windows, while Bybit end-only pagination can span gaps. Every page renders progressively; prepends shift the viewport and active drag origin by the number of new candles, preserving candle anchors. If all four pages each add 240 real candles and advance the cursor, remaining demand continues after 500ms. Repeated zoom-out retains pending demand, while reversing to zoom-in immediately uses the visible range. A failed page preserves the chart and cursor for retry; sparse/empty scans retain the bounded continue action when demand remains. These operations never alter or reconnect `PriceFeed`.
...
- `createCandleHistoryLoader` freezes one selection/interval and delivers nonempty Coinbase/Gate pages without first querying origin metadata, including short pages. It lazily caches verified origin metadata on the first empty page, so origin discovery cannot delay rendering already received candles. Bybit uses end-only pagination so a valid empty result covers all older times. Completion stops both demand and prefetch; unknown origin, explicit cache limit, malformed response, and exhausted history remain distinct. Metadata 429/403 preserve existing candles and signal a one-time cooldown. See `MARKET_DATA.md` for provider-specific proof and limits.
- Date/price ticks and crosshair annotations use matching measured 12px semibold fonts. Tick color is `#b2b5be`; crosshair labels use white on gray `#4c525e`. Price grid ticks use bounded 1/2/2.5/5/10 decimal steps at roughly 64px spacing; optional horizontal and vertical grid lines remain subdued. The helper changes tick placement, not the padded OHLC price scale. The plot reserves 104px on the right (96px below 520px) and 40px below for legible, unclipped labels. Candle bodies, OHLC values, current-price line and axis marker share green `#26a69a` / red `#ef5350`; stale-price presentation stays gray.
...
- `ChartNavigation.startPrefetch()` targets two viewport widths of older candles before the visible left edge (`ceil(viewport.count × 2)`, clamped to 480–1,920), usually warming an initial 240-candle page to 720 cached candles while still displaying the same latest 120. Speculative pages wait 250ms after completion and share one flight with prioritized demand. Empty/nonadvancing/error responses or four insufficient sparse pages pause speculation until successful explicit demand; full productive pages do not consume the sparse-page budget. HTTP 429/403 block older requests in that navigation instance for 60 seconds/10 minutes. Cancellation clears timers and aborts obsolete requests. This does not prefetch other products, intervals, or exchanges.
- Failed/nonadvancing demand responses and unfinished batches containing sparse/empty pages pause automatic demand, so repeated pointer moves cannot create a retry burst. Only four fully productive pages permit a paced automatic continuation, which remains visibly loading while queued. Reset, reverse navigation, End and cancellation revoke obsolete queued demand. Explicit continuation retries paused demand subject to cooldown; retreating into loaded data or resetting clears that demand pause. Quiet prefetch failures still permit one later foreground attempt.
